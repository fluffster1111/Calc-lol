...
MainFrame::MainFrame(const wxString& title)
	: wxFrame(nullptr, wxID_ANY, title), output(nullptr), sign(nullptr), num1(nullptr), num2(nullptr), mult(nullptr), divide(nullptr), plus(nullptr), minus(nullptr), calc(nullptr), ans(nullptr), power(nullptr), root(nullptr), power2(nullptr), clear(nullptr)
{
	wxPanel* panel = new wxPanel(this);

	output = new wxStaticText(panel, OUTPUT_ID, "", wxPoint(100, 100), wxSize(600, 50), wxALIGN_CENTER | wxALIGN_CENTER_VERTICAL);
	output->SetBackgroundColour(*wxLIGHT_GREY);
	wxFont font1(24, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_BOLD);
	output->SetFont(font1);

	choices.Add("Operators");
	choices.Add("+");
	choices.Add("-");
	choices.Add("x");
	choices.Add("/");
	...
	power = new wxButton(panel, POWER_ID, "x^x", wxPoint(340, 220), wxSize(50, 50));
	root = new wxButton(panel, ROOT_ID, "root", wxPoint(400, 220), wxSize(50, 50));
	power2 = new wxButton(panel, POWER2_ID, "x^2", wxPoint(280, 220), wxSize(50, 50));
	clear = new wxButton(panel, CLEAR_ID, "CLEAR", wxPoint(460, 220), wxSize(50, 50));

	calc = new wxButton(panel, BUTTON_ID, "calc", wxPoint(350, 350), wxSize(100, 50));

	ans = new wxButton(panel, ANS_ID, "ANS", wxPoint(150, 350), wxSize(100, 30));
}
...
void MainFrame::calculate(wxCommandEvent& evt)
{
	...
	if (GetNum1.ToDouble(&number1)) {
		...
		wxMessageBox("Invalid number entered. Please try again.", "Error", wxOK | wxICON_ERROR);
	}
	...
	if (choice == "+") {
		...
		output->SetLabel(wxString::Format("%.4f", result));
	}
	else if (choice == "-") {
		result = number1 - number2;
		output->SetLabel(wxString::Format("%.4f", result));
...
void MainFrame::ANS(wxCommandEvent& evt) {
	number1 = result;
	num1->SetLabel(wxString::Format("%.4f", number1));
